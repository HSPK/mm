...
def import_cmd(source: Path, move: bool, metadata_mode: str) -> None:
    ...
    try:
        ...
        ui.success(
            f"{'Moved' if move else 'Copied'} {result.file_count:,} file(s); "
            f"indexed {result.indexed_count:,} in the database."
        )
    finally:
        active.close()
