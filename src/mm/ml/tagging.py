...
def add_tags(db: AsyncDBClient | Any, media_id: int, tag_names: list[str]) -> None:
    """Manually attach tags to a media entry."""
    for name in tag_names:
        tag = db.tag.get_or_create(name, source=TagSource.MANUAL)
        assert tag.id is not None
        db.tag.add_media(media_id, tag.id, confidence=1.0)


def remove_tags(db: AsyncDBClient | Any, media_id: int, tag_names: list[str]) -> None:
    """Remove tags from a media entry."""
    for name in tag_names:
        tag = db.tag.get_by_name(name)
        if tag and tag.id is not None:
            db.tag.remove_media(media_id, tag.id)
...
def apply_rule_tags(
    ...
    def _add(name: str) -> None:
        tag = db.tag.get_or_create(name, source=TagSource.RULE)
        assert tag.id is not None
        assert media.id is not None
        db.tag.add_media(media.id, tag.id, confidence=1.0)
        added.append(name)
...
def apply_clip_tags(
    ...
    for label, sim in zip(labels, sims):
        conf = float(sim)
        if conf >= threshold:
            tag = db.tag.get_or_create(label, source=TagSource.AUTO_CLIP)
            assert tag.id is not None
            db.tag.add_media(media.id, tag.id, confidence=conf)
