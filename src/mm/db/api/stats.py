"""Aggregation / statistics database API."""
...
class StatsApi(DbApi):
    ...
    async def by_year(self) -> list[dict[str, Any]]:
    ...
    async def by_camera(self) -> list[dict[str, Any]]:
    ...
    async def by_extension(self) -> list[dict[str, Any]]:
    ...
    async def completeness(self) -> dict[str, int]:
        """Counts for metadata completeness: has/missing date, GPS, camera, tags."""
        alive = MediaModel.deleted_at.is_null()
        total = await self.objects.count(MediaModel.select().where(alive))
    ...
    async def top_tags(self, limit: int = 15) -> list[dict[str, Any]]:
        """Most-used tags."""
        rows = await self.objects.fetchall(
            TagModel.select(
                TagModel.name,
